...
function sendUpdatedBookData(formId){
	var xhttp;
	if (window.XMLHttpRequest) {
		xhttp = new XMLHttpRequest();
	} 
	...
	var id_book_input = form.getElementsByTagName('input')[0];
	var book_title_input = form.getElementsByTagName('input')[1];
	var original_book_title_input = form.getElementsByTagName('input')[2];
	var category_select = form.getElementsByTagName('select')[0];
	var category_id = category_select.options[category_select.selectedIndex].value;
	var author_selects = form.getElementsByTagName('ul')[0].getElementsByTagName('li');
	var genre_selects = form.getElementsByTagName('ul')[1].getElementsByTagName('li');
	
	var params = 'id-book='+id_book_input.value+'&book-title-input='+book_title_input.value+'&original-book-title-input='+original_book_title_input.value+'&id-category='+category_id+"&author-size="+author_selects.length;
