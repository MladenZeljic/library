...
function sendUpdatedCopyData(modalId){
	var xhttp;
	if (window.XMLHttpRequest) {
		xhttp = new XMLHttpRequest();
	} 
	...
	var copy_id_input = modal.getElementsByTagName('input')[0];
	var year_of_publication_input = modal.getElementsByTagName('input')[1];
	var number_of_pages_input = modal.getElementsByTagName('input')[2];
	var book_select = modal.getElementsByTagName('select')[0];
	var book_id = book_select.options[book_select.selectedIndex].value;
	var publisher_select = modal.getElementsByTagName('select')[1];
	var publisher_id = publisher_select.options[publisher_select.selectedIndex].value;
	
	var params = 'id-copy='+copy_id_input.value+'&year-of-publication-input='+year_of_publication_input.value+'&number-of-pages-input='+number_of_pages_input.value+'&id-book='+book_id+"&id-publisher="+publisher_id;	
